    ...
    while True:
        if classed_tokens[token_idx].tokenVal == "sub":
            ...
            if classed_tokens[token_idx + 1].tokenType == "CONDITION_WRAPPER":
                ...
                raise SyntaxError(
                    "If statement is not followed by condition wrapper '|'"
                )
            token_idx += 1
...
def toNewick(l: list[Node]):
    ...
    for idx, node in enumerate(l):
        if len(node.connectedLowerNodes) > 0:
            t_tuple.append(
                [node.tokenDetails.tokenType, toNewick(node.connectedLowerNodes)]
            )
        else:
...
def cleanInputs(tokenList: list[tokenType]) -> list:
    ...
    while True:
        inputs = [
            idx for idx, token in enumerate(tokenList) if token.tokenVal == "askInput"
        ]
        if len(inputs) == 0:
            break
        idx = inputs[0]
        ...
        lastLineSearch = [
            idx
            for idx, token in enumerate(tokenList[:idx])
...
def cleanForLoops(tokenList: list[tokenType]) -> list:
    ...
    while True:
        forLoops = [
            idx for idx, token in enumerate(tokenList) if token.tokenVal == "for"
        ]
        if len(forLoops) == 0:
            break
        ...
        findArgumentEnd = [
            i + idx + 2
            for i, token in enumerate(tokenList[idx + 2 :])
            if token.tokenType == "ARGUMENTS_END"
        ]
        separatedConditions = list(
            list(g)
            for k, g in itertools.groupby(
                tokenList[idx + 2 : findArgumentEnd[0]],
                lambda x: x.tokenType not in ["ARGUMENTS_SEP"],
            )
            if k
        )
        ...
        tokenList[idx : tokenIdx + 1] = (
            forLoopStart
            + [tokenType(("END_LINE", -1, ";")), tokenType(("STATEMENT", -1, "WHILE"))]
            + forLoopCondition
            + [tokenType(("START_BLOCK", -1, "{"))]
            + forLoopContents
            + forLoopIterator
            + [tokenType(("END_LINE", -1, ";")), tokenType(("END_BLOCK", -1, "WEND"))]
        )
...
def cleanWhile(tokenList: list[tokenType]) -> list:
    ...
    while True:
        whiles = [
            idx for idx, token in enumerate(tokenList) if token.tokenVal == "while"
        ]
        if len(whiles) == 0:
            break
        ...
        findConditionWrapper = [
            i + idx + 2
            for i, token in enumerate(tokenList[idx + 2 :])
            if token.tokenType == "CONDITION_WRAPPER"
        ]
...
def cleanIfStatement(tokenList: list) -> list:
    ...
    while True:
        ifStatements = [
            idx for idx, token in enumerate(tokenList) if token.tokenVal == "if"
        ]
...
def transpile(tokenList: list) -> str:
    ...
    # noIfStatements = 0
    endLineStatementPos = [
        n for n, t in enumerate(tokenList) if t.tokenType == "END_LINE"
    ]
    finishedLines = 0
    while True:
        if tokenList[tokenIdx].tokenVal == "sub":
            ...
            temp = list(
                list(g)
                for k, g in itertools.groupby(
                    tokenList[tokenIdx + 2 : endLineStatementPos[finishedLines] - 1],
                    lambda x: x.tokenType not in ["ARGUMENTS_SEP"],
                )
                if k
            )
            ...
            subroutines[subName].append(f"PRINT {string};")
            ...
            subroutines[subName].append(f"PRINT clear$;")
            tokenIdx += endLineStatementPos[finishedLines] - tokenIdx
        elif tokenList[tokenIdx].tokenVal == "enableCursor":
            subroutines[subName].append(f"PRINT con$;")
            tokenIdx += endLineStatementPos[finishedLines] - tokenIdx
        elif tokenList[tokenIdx].tokenVal == "disableCursor":
            subroutines[subName].append(f"PRINT coff$;")
            tokenIdx += endLineStatementPos[finishedLines] - tokenIdx
        elif tokenList[tokenIdx].tokenVal == "resetCursor":
            subroutines[subName].append(f"PRINT home$;")
            tokenIdx += endLineStatementPos[finishedLines] - tokenIdx
            ...
            temp = list(
                list(g)
                for k, g in itertools.groupby(
                    tokenList[tokenIdx + 1 : endLineStatementPos[finishedLines]],
                    lambda x: x.tokenType not in ["ASSIGN"],
                )
                if k
            )
            temp = [transpileFlat(t_list) for t_list in temp]
            ...
            subroutines[subName].append(f"{varName} = {varValue}")
            tokenIdx += endLineStatementPos[finishedLines] - tokenIdx
            ...
            subroutines[subName].append(f"INPUT {inputString}, {variableName}")
            tokenIdx += endLineStatementPos[finishedLines] - tokenIdx

        elif tokenList[tokenIdx].tokenVal == "WHILE":
            findConditionWrapper = [
                i + tokenIdx + 2
                for i, token in enumerate(tokenList[tokenIdx + 2 :])
                if token.tokenType == "CONDITION_WRAPPER"
            ]

            conditions = tokenList[tokenIdx + 2 : findConditionWrapper[0]]
            subroutines[subName].append(f"WHILE {' '.join(transpileFlat(conditions))}")

            tokenIdx += len(conditions) + 2

        elif tokenList[tokenIdx].tokenVal == "WEND":
            subroutines[subName].append(f"WEND")
            tokenIdx += 1
            ...
            if tokenList[tokenIdx + 1].tokenType != "VARIABLE":
                raise SyntaxError("NO VARIABLE / subName FOLLOWING CALL COMMAND")
    ...
    return [
        f"REM SUB:{key}\n" + "\n".join(text) + "\nRETURN"
        for key, text in subroutines.items()
    ]
...
def numberLines(transpiledCode: list[str]) -> str:
    ...
    regex = r"REM SUB:([\w\d]+)"
    subroutinesDictionary = {
        re.match(regex, line).group(1): lineNo
        for lineNo, line in enumerate(transpiledCode)
        if re.match(regex, line)
    }
    for subName, subPos in subroutinesDictionary.items():
        transpiledCode = [
            line.replace(f"replaceCall {subName}", f"GOSUB {subPos*10}")
            for line in transpiledCode
        ]
